function simulate()
   ...
   # end
   #
   # open("train.csv", "w") do io
   #    writedlm(io, formatted_data, ',')
   # end;

   n_rows = length(data)

   single_sample_data = zeros(n_rows, 9)

   for i=1:n_rows
      single_sample_data[i, :] = data[i]
   end

   open("rnn_test.csv", "w") do io
      writedlm(io, single_sample_data, ',')
   end;
end
